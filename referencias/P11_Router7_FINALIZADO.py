...
###################################################
	...
	def natTraductor(self, sourceIp, destinationIp, sourcePort, destinationPort):
		...
		if(destinationIp == self.interfaces[4][0]):	#Si el paquete entra desde internet
			...
			if (sourceIp,sourcePort) in self.natbyIp.keys():	#Si la ip esta en la tabla NAT
				newSourcePort = self.natbyIp[(sourceIp, sourcePort)]
	...
	def handle_nat(self, datapath, msg, pkt):
		...
		if pkt.get_protocol(tcp.tcp) != None:	#Si pkt es TCP
			...
			if(msg.match['in_port'] == 4): 	#Si entra desde internet
				sourceIp 		= ipv4_pkt.src
				destinationIp 	= ipv4_pkt.dst
				...
				if(dstMac != None):
					match = datapath.ofproto_parser.OFPMatch(ipv4_src=sourceIp,ipv4_dst=destinationIp,tcp_dst=destinationPort ,eth_type=ether.ETH_TYPE_IP, ip_proto=0x06)
					actions = [datapath.ofproto_parser.OFPActionSetField(eth_src=self.macs[port]),
							   datapath.ofproto_parser.OFPActionSetField(eth_dst=dstMac),
							   datapath.ofproto_parser.OFPActionSetField(ipv4_dst=newDestinationIp),
				...
				if(dstMac != None):
					...
					match = datapath.ofproto_parser.OFPMatch(ipv4_src=sourceIp,ipv4_dst=destinationIp,tcp_src=sourcePort ,eth_type=ether.ETH_TYPE_IP, ip_proto=0x06)
					actions = [datapath.ofproto_parser.OFPActionSetField(eth_src=self.macs[port]),
							   datapath.ofproto_parser.OFPActionSetField(eth_dst=dstMac),
							   datapath.ofproto_parser.OFPActionSetField(ipv4_src=newSourceIp),
			...
			if(port == 4):	#Sale a internet
				...
				if(dstMac != None):
					...
					match = datapath.ofproto_parser.OFPMatch(ipv4_src=sourceIp,ipv4_dst=destinationIp,udp_dst=destinationPort ,eth_type=ether.ETH_TYPE_IP, ip_proto=0x11)
				...
				if(dstMac != None):
					match = datapath.ofproto_parser.OFPMatch(ipv4_src=sourceIp,ipv4_dst=destinationIp, udp_src=sourcePort ,eth_type=ether.ETH_TYPE_IP, ip_proto=0x11)
