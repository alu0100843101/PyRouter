...
class L2Forwarding(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.natbyPublicId = {}	#Tabla nat para ICMP
		self.natbyIpICMP = {}	#Tabla nat para ICMP
...
###################################################
	...
	def natTraductorICMP(self, sourceIp, destinationIp, sourceId, destinationId):
		print "SourceIp: %s  DestinationIp: %s  DestinationId: %s  SourceId: %s" % (sourceIp, destinationIp, sourceId, destinationId)
		if(destinationIp == self.interfaces[4][0]):	#Si el paquete entra desde internet
			#if destinationPort in self.natbyPublicPort.keys():
			destinationIp 		= self.natbyPublicId[id][0]
			destinationId 		= self.natbyPublicId[id][1]
		else:									#Si el paquete sale a internet
			newSourceIp = self.interfaces[4][0]
			if sourceIp in self.natbyPublicId.keys():	#Si la ip esta en la tabla NAT
				newSourceId = self.natbyPublicId[sourceIp][1]
			else:										#Si no está en la tabla

				newSourceId = randint(0,65535)		#1024-1124

				while newSourceId in self.natbyPublicId.keys():
					newSourcePId = randint(0,65535)		#1024-1124
				self.natbyIpICMP[sourceIp] 				= [sourceId, newSourceId]
				self.natbyPublicId[newSourceId] = [sourceId, sourceIp]


			sourceIp 	= newSourceIp
			sourceId 	= newSourceId

		print "NEW SourceIp: %s  destinationIp: %s  sourceId: %s  destinationId: %s" % (sourceIp, destinationIp, sourceId, destinationId)
		return(sourceIp,destinationIp,sourceId,destinationId)
	...
	def handle_nat(self, datapath, msg, pkt):
		...
		if pkt.get_protocol(icmp.icmp) != None:	#Si pkt es ICMP
			...
			if(msg.match['in_port'] == 4): 
				...
				(newSourceIp, newDestinationIp, newSourcePort, newDestinationPort) = self.natTraductorICMP(sourceIp, destinationIp, None, None)
				...
				if(dstMac != None):
					match = datapath.ofproto_parser.OFPMatch(ipv4_src=sourceIp,ipv4_dst=destinationIp ,eth_type=ether.ETH_TYPE_IP)
					actions = [datapath.ofproto_parser.OFPActionSetField(eth_src=self.macs[port]),
							   datapath.ofproto_parser.OFPActionSetField(eth_dst=self.iptoMac[destinationIp]),
				...
				if(dstMac != None):
					...
					(newSourceIp, newDestinationIp, foo1, foo2) = self.natTraductorICMP(sourceIp, destinationIp, None, None)
...
'''
		match = datapath.ofproto_parser.OFPMatch(ipv4_dst=pkt_ipv4.dst ,eth_type=ether.ETH_TYPE_IP)
				actions = [datapath.ofproto_parser.OFPActionSetField(eth_src=self.macs[selectNetwork[2]]),
						   datapath.ofproto_parser.OFPActionSetField(eth_dst=self.iptoMac[dstIPtoMAC]),
						   datapath.ofproto_parser.OFPActionDecNwTtl(),
